...
## Types of Conditions
Conditions within an Action Block are essential to determine whether the actions should be executed or skipped. They allow you to create dynamic and flexible automations that respond to specific criteria or situations within your ERPLite app.
...
By using sequential conditional execution, you can create powerful and flexible automations that respond to specific scenarios or combinations of criteria within your ERPLite app. This enhances the app's overall functionality and provides a more engaging and dynamic user experience.
...
By using multiple actions within an Action Block and utilizing the "Exit on error" feature, you can create more robust and error-resistant automations, ultimately improving the efficiency and reliability of your ERPLite app.

## Multiple Action Blocks in an Automation
In ERPLite, an automation must have at least one Action Block to define the actions that will be performed when the automation is triggered. However, you can also include multiple Action Blocks within a single automation to enable complex and flexible logic based on different sets of conditions.
...
This can be useful for handling different scenarios or combinations of criteria within your ERPLite app, as the automation can respond to multiple situations with different actions.
...
interface Condition {
  conditionId: string;
  type: 'EXPRESSION' | 'REST_API' | 'LOCATION_CHECK';
...
interface ExpressionCondition {
  type: 'EXPRESSION';
  expression: string;  // JavaScript returning boolean
}

interface RestApiCondition {
  type: 'REST_API';
  url: string;
  method: string;
...
interface LocationCheckCondition {
  type: 'LOCATION_CHECK';
  location1: string;
  location2: string;
...
interface UpdateDataFieldsAction {
...
|-----------|----------|---------|
| ActionBlockEditor | `/src/views/automations/ActionBlock/` | Block configuration |
| ConditionEditor | `/src/views/automations/ActionBlock/Condition/` | Condition setup |
| ActionEditor | `/src/views/automations/ActionBlock/Action/` | Action configuration |
| RestApiConfig | `/src/views/automations/RestApi/` | API call setup |
...
2. If any condition returns false → skip entire block
3. If all conditions pass → execute actions sequentially
4. If action fails and exitOnError → stop remaining actions
5. Move to next action block
```